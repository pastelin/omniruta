...
@RequestMapping("/api/legal/client")
public class HEADLegalClientController {
    ...
    public ResponseEntity<HEADApiResponse<HEADLegalAcceptanceResponse>> acceptDocument(
        ...
        return ResponseEntity.ok(HEADApiResponse.ok(headAcceptLegalDocumentService.acceptForClient(request)));
    }

    @GetMapping("/status/document")
    public ResponseEntity<HEADApiResponse<HEADLegalAcceptanceStatusResponse>> getStatusDocument() {
        return ResponseEntity.ok(HEADApiResponse.ok(legalAcceptanceStatusService.getClientStatus()));
    }
}
